...
describeWin32("tray launcher: root shortcut → environment + tray icon", () => {
  ...
  // The kit-synced engine header identifies it as the shared source of truth (proves
  // this copy wasn't hand-edited into a fork) and the adapter dot-sources it.
  it("the engine file carries the kit-synced header and the adapter dot-sources it", () => {
    const engine = fs.existsSync(enginePath) ? fs.readFileSync(enginePath, "utf8") : "";
    expect(/synced into each app/i.test(engine)).toBe(true);
    expect(/DO NOT EDIT THIS FILE INSIDE AN APP/i.test(engine)).toBe(true);
  ...
  // The shared launcher (.vbs) and shortcut engine (New-TrayShortcut.ps1) carry the same
  // kit-synced provenance header as Tray-Host.ps1, and Create-Shortcut.ps1 dot-sources the
  // engine rather than building the .lnk itself - proving neither was hand-edited into a
  // per-app fork.
  it("the vbs launcher and shortcut engine carry the kit-synced header; Create-Shortcut dot-sources the engine", () => {
    // "synced into each\n' app's misc/" (vbs) / "synced into each\n# app's misc/" (ps1) wraps
    // across a line in these two files (unlike Tray-Host.ps1, where it's a single line) -
    // tolerate the intervening newline + comment-marker prefix of the wrapped continuation.
    const vbs = fs.existsSync(vbsPath) ? fs.readFileSync(vbsPath, "utf8") : "";
    expect(/synced into each\s*['#]?\s*app/i.test(vbs)).toBe(true);
    expect(/DO NOT EDIT THIS FILE INSIDE AN APP/i.test(vbs)).toBe(true);

    const shortcutEngine = fs.existsSync(shortcutEnginePath) ? fs.readFileSync(shortcutEnginePath, "utf8") : "";
    expect(/synced into each\s*['#]?\s*app/i.test(shortcutEngine)).toBe(true);
    expect(/DO NOT EDIT THIS FILE INSIDE AN APP/i.test(shortcutEngine)).toBe(true);
  ...
  // ReDesign's rebuild resolver (misc\Rebuild.bat first, else npm run build) and its always-on
  // dev-tree gate (no dev/distribution split) are ADAPTER config; the dual/triple gate that
  // hides the menu item outside a dev tree is ENGINE-invariant machinery.
  it("supplies a Rebuild.bat-first resolver and always shows Rebuild & Restart", () => {
    const tray = fs.existsSync(trayPath) ? fs.readFileSync(trayPath, "utf8") : "";
    expect(/Rebuild\.bat/i.test(tray)).toBe(true);
    expect(/npm run build/i.test(tray)).toBe(true);
    expect(/IsDevTree\s*=\s*\$true/i.test(tray)).toBe(true);
