// Applies the launcher's window-size hint: the `?window-size=WxH` param the daemon, the tray
// and start.cmd's Open-Ui.ps1 append to a portable window's URL. When a portable window is
// launched while another window on the profile is already open, Chromium FORWARDS the launch
// into the running instance and the new window inherits THAT window's geometry —
// `--window-size` and the saved placement are both ignored (verified Edge 150, 2026-07-16).
// Nothing outside the page can fix that, so the launcher tags the URL with the size the
// window should have and the page corrects itself here, once, at startup.
//
// The param name and the "WxH" shape are a contract shared with the daemon
// (src/window-size.ts) and the tray (misc/Tray-Host.ps1) — keep them in step. ReDesign has
// no web+server shared module, so the literal and the parser live here too rather than being
// imported (the daemon holds its own copies).
