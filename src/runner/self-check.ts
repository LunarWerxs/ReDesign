...
export async function captureSelfCheck(outputHtml: string): Promise<{ images: LoadedImage[]; files: string[] }> {
  const images: LoadedImage[] = [];
  const files: string[] = [];
  for (const viewport of SELF_CHECK_VIEWPORTS) {
    const png = selfCheckCapturePath(outputHtml, viewport.name);
    await renderHtmlToPng(outputHtml, png, { width: viewport.width, height: viewport.height, fullPage: true, mobile: viewport.mobile });
