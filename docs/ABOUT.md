...
- The Windows system-tray icon and portable chromeless window mode live in a separate small Rust binary under misc/tray-host-native, not the main app - running the packaged exe by itself never spawns a tray icon. anchors: `misc/tray-host-native/src/daemon.rs:126`
...
- **System tray icon (Windows)** - An optional native tray host (separate small Rust launcher) gives RēDesign a system-tray icon and a portable chromeless window mode; running the main exe alone never spawns one. - `misc/tray-host-native/src/browser.rs:121`, `misc/tray-host-native/src/daemon.rs:126`
